...
Output: logs/<tool>/<session_id>.jsonl  (tool = claude-code | codex | cursor).
...
def _cursor_has_text(obj) -> bool:
    """True if a Cursor transcript line is a user/assistant turn with conversation text."""
...
def slim_transcript(raw: str, tool: str):
    ...
    if tool == "codex":
        ...
        kept = [line for line, obj in pairs if _cursor_has_text(obj)]
    else:
...
def main() -> int:
    ...
    parser.add_argument("--tool", required=True, choices=["claude-code", "codex", "cursor"])
